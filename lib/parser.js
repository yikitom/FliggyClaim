...
(function (global) {
  ...
  // Each pattern must be specific enough to be conclusive on its own — the
  // default is CNY, so we never want to flip the currency away from CNY without
  // an unambiguous match. Bare "$" / "¥" are intentionally not listed here
  // because they're shared between USD/HKD/SGD and CNY/JPY respectively.
  const CURRENCY_MAP = [
    [/(?:￥|RMB|\bCNY\b|人民币|元)/i, "CNY"],
    [/(?:US\$|\bUSD\b|美元|美金)/i, "USD"],
    [/(?:€|\bEUR\b|欧元)/i, "EUR"],
    [/(?:£|\bGBP\b|英镑)/i, "GBP"],
    [/(?:\bJPY\b|日元|円)/i, "JPY"],
    [/(?:HK\$|\bHKD\b|港币|港元)/i, "HKD"],
    [/(?:S\$|\bSGD\b|新币|新加坡元)/i, "SGD"],
  ];
  ...
  function detectAmount(text) {
    ...
    // Bare "$" is intentionally omitted — too ambiguous, and we'd rather fall
    // back to the CNY default than mislabel a CNY receipt as USD.
    const candidates = [];
    const re =
