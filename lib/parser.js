...
(function (global) {
  const TYPE_KEYWORDS = {
    ...
    taxi: [
      ...
      "ride",
      "首汽",
      // Was "T3", which scored a taxi point for every 「T3航站楼」 on an
      // airport meal or flight receipt.
      "t3出行",
      "行程",
      "里程",
    ],
  ...
  async function parseFile(file, opts) {
    ...
    // Try optional remote OCR refinement. Everything it returns is validated
    // before it can touch the record — see sanitizeRefined.
    let amountSource = heuristic.amountSource || "none";
    const refined = sanitizeRefined(
      await safe(() => remoteRefine(file, { type, date, amount, currency }), null),
    );
    if (refined) {
      ...
      if (refined.amount != null) {
        amount = refined.amount;
        // Label the source accurately: the popup warns about weak sources, and
        // it used to keep saying "filename" after the endpoint had replaced
    ...
    if (refined) {
      ...
      if (refined.nights) {
        rec.nights = refined.nights;
        if (rec.type === "hotel") rec.checkout = addDays(rec.checkin || date, refined.nights);
      }
      if (refined.checkin) rec.checkin = refined.checkin;
  ...
  function extractNights(text) {
    // The old pattern ended in \b, which never matches after 晚 (a non-word
    // char) unless a word char happens to follow: "深圳酒店1晚68444" worked
    // only because of the digits after it, while "住宿 2晚" returned null and
    // the checkout date silently fell back to checkin + 1. Keep the boundary
    // on the ASCII alternatives only, and drop the bare "n" (pure noise).
    const m = text.match(/(\d{1,2})\s*(?:晚|夜|nights?\b|nts?\b)/i);
  ...
  // All-UTC arithmetic on a plain Y-M-D. (The previous version mixed a UTC
  // parse with local setDate; it happened to come out right in every zone,
  // but only because the two offsets cancel — not worth relying on.)
  ...
  /**
   * Does `word` occur in `lower` (already lowercased)?
   *
   * CJK keywords match as plain substrings, but ASCII ones need boundaries:
   * "inn" is inside "dinner", so a terse 「DINNER RECEIPT」 scored a point for
   * BOTH hotel and meal and — on the tie — came out as 差旅-住宿. Likewise
   * "cab" is inside "cabin" on every boarding pass.
   */
  function keywordHit(lower, word) {
    const w = word.toLowerCase();
    if (!/[^\x00-\x7F]/.test(w)) {
      return new RegExp(`(?<![a-z0-9])${escapeRe(w)}(?![a-z0-9])`).test(lower);
    }
    return lower.includes(w);
  }

  function scoreType(lower, words) {
    let hits = 0;
    let specificity = 0;
    for (const w of words) {
      if (!keywordHit(lower, w)) continue;
      hits++;
      specificity += w.length;
    }
  ...
  function detectType(text) {
    ...
    let bestType = null;
    let best = { hits: 0, specificity: 0 };
    for (const [type, words] of Object.entries(TYPE_KEYWORDS)) {
      const s = scoreType(lower, words);
      if (s.hits === 0) continue;
      // Tie on hit count → prefer the more specific evidence ("电子客票" beats
      // a lone "行程"), instead of silently falling back to key order.
      if (s.hits > best.hits || (s.hits === best.hits && s.specificity > best.specificity)) {
        best = s;
        bestType = type;
      }
    }
  ...
  function guessTypeFromFilename(name) {
    ...
    for (const [type, words] of Object.entries(TYPE_KEYWORDS)) {
      for (const w of words) {
        if (keywordHit(lower, w)) return type;
      }
    }
    return null;
  }
  ...
  const SUPPORTED_TYPES = ["flight", "hotel", "meal", "taxi", "train", "other"];
  const SUPPORTED_CURRENCIES = ["CNY", "USD", "EUR", "JPY", "HKD", "GBP", "SGD"];

  // The endpoint is whatever the user typed into Options. Receipt images and
  // the Bearer key must not leave over plaintext http — allow it only for a
  // self-hosted service on this machine.
  ...
  /**
   * The refine endpoint is arbitrary third-party code and its answer lands in
   * a real expense claim, so treat the response as untrusted input: every
   * field is whitelisted or dropped. Previously `type/date/amount/currency`
   * were copied over verbatim, so a malformed reply could put "abc" in the
   * amount or an unknown code in 币种 and we'd carry it all the way to TAE.
   */
  function sanitizeRefined(raw) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
    const str = (v, max) => (typeof v === "string" ? v.trim().slice(0, max) : null);
    const out = {};

    if (SUPPORTED_TYPES.includes(raw.type)) out.type = raw.type;

    const date = str(raw.date, 10);
    if (isValidIsoDate(date)) out.date = date;

    const amount = typeof raw.amount === "number" ? raw.amount : parseFloat(str(raw.amount, 20) || "");
    if (isPlausibleAmount(amount)) out.amount = amount;

    const cur = (str(raw.currency, 8) || "").toUpperCase();
    if (SUPPORTED_CURRENCIES.includes(cur)) out.currency = cur;

    const city = str(raw.city, 20);
    if (city) out.city = city;

    const nights = Math.trunc(Number(raw.nights));
    if (Number.isFinite(nights) && nights >= 1 && nights <= 60) out.nights = nights;

    const checkin = str(raw.checkin, 10);
    if (isValidIsoDate(checkin)) out.checkin = checkin;
    const checkout = str(raw.checkout, 10);
    if (isValidIsoDate(checkout)) out.checkout = checkout;
  ...
  async function remoteRefine(file, draft) {
    ...
    if (!isSafeEndpoint(cfg["fliggy.ocr.endpoint"])) {
      console.warn(
        "[FliggyParser] 远程识别接口必须是 https（本机 http://localhost 除外），已跳过:",
        cfg["fliggy.ocr.endpoint"],
      );
      return null;
    }
  ...
  // LOCAL today. toISOString() is UTC, so for a UTC+8 user importing between
  // midnight and 08:00 it handed out yesterday's date — and this is the date
  // every receipt with no detectable date gets stamped with.
  function todayISO() {
    const d = new Date();
